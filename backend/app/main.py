...
from app.api.routes import auth, catalog, downloads, users, media, systems_config, bugreports, debug, collections, tracker, tracker_admin
import logging
import os
import sys
import asyncio
...
app.include_router(tracker.router, prefix="/api/internal/tracker", tags=["tracker-internal"])
app.include_router(tracker_admin.router, prefix="/api/admin/tracker", tags=["tracker-admin"])
...
async def cleanup_stuck_downloads():
    ...
    - Downloads older than MAX_DOWNLOAD_AGE_HOURS that are still stuck -> cancelled permanently
    ...
    while True:
        try:
            ...
            try:
                ...
                # Pass 1: process downloads detected as stuck via Redis scan
                if stuck_downloads_redis:
                    redis_stuck = db.query(DownloadQueue).filter(
                        DownloadQueue.id.in_(stuck_downloads_redis.keys())
                    ).all()
