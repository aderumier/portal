...
class UPnPHelper:
    ...
    async def discover_router(self) -> bool:
        ...
        try:
            ...
            # Get local IP from UPnP lanaddr (always returns local IP, not router IP)
            try:
                if hasattr(self.upnp, 'lanaddr') and self.upnp.lanaddr:
                    self.local_ip = self.upnp.lanaddr
                    logger.info(f"Found local IP from UPnP lanaddr: {self.local_ip}")
            except Exception as e:
                logger.debug(f"Could not get local IP from UPnP lanaddr: {e}")
    ...
    async def _get_local_ip_for_router(self) -> Optional[str]:
        ...
        """
        # Return the local IP from lanaddr (set during router discovery)
        if self.local_ip:
            return self.local_ip
        
        logger.warning("Local IP not available from UPnP lanaddr")
        return None
